package com.ideaaedi.zoo.diy.artifact.auth.satoken.processor;

import com.ideaaedi.zoo.commonbase.processor.ZooEnvironmentPostProcessor;

/*
 * 默认配置文件加载
 */
public class ZooSaTokenEnvProcessor extends ZooEnvironmentPostProcessor {
    
    @Override
    protected String propertySourceName() {
        return "zoo-diy-artifact-sa-token.default";
    }
    
    @Override
    protected String propertySourceClasspath() {
        return "zoo-diy-artifact-sa-token.default.yaml";
    }
    
    @Override
    protected boolean highestPriority() {
        return false;
    }
}
