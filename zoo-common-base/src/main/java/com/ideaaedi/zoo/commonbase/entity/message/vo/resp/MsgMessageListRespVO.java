 ...
 */
@Data
@Schema(description = "消息-内容表 list resp")
public class MsgMessageListRespVO {
    ...
    @ExcelProperty("消息类型(0-即时消息; 1-定时消息)")
    ...
    @ExcelProperty("消息过期时间(单位秒; 0-永不过期)")
    ...
    @ExcelProperty("消息状态（负数表示执行前状态，正数表示执行后状态：-3-草稿态；-2-已失效；-1-已过期；0-待执行；1-执行成功；2-执行失败；3-执行中）")
