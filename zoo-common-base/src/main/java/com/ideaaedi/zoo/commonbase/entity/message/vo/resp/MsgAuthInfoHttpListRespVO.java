 ...
 */
@Data
@Schema(description = "消息-http(s)认证表 list resp")
public class MsgAuthInfoHttpListRespVO {
    ...
    @ExcelProperty("auth类型（ACT_PWD：账密；AK_SK：客户id与秘钥；TOKEN-凭据）")
    ...
    @ExcelProperty("请求auth url时的content-type")
    ...
    @ExcelProperty("请求auth url时的header（支持spring占位符${}解析，可选的占位符变量见auth_vars、auth_sys_vars）")
    ...
    @ExcelProperty("请求auth url时的body（支持spring占位符${}解析，可选的占位符变量见auth_vars、auth_sys_vars）")
    private String body;
    ...
    @ExcelProperty("auth变量名与变量值，json对象")
    private String vars;
    ...
    @ExcelProperty("需要的系统变量，json数组（RANDOM32STR-32位随机字符串；NOW_SECOND-当前到秒的时间戳；NOW-当前到毫秒的时间戳）")
    ...
    @ExcelProperty("auth文档（markdown格式）")
    private String doc;
    ...
    @ExcelProperty("auth状态(1-正常；2-禁用)")
